...
{
    public class LandRepository(LandContext context) : ILandRepository
    {
        ...
        public async Task<List<LandData>> GetLands(string[] producesAny, string[] doesNotProduceAny)
        {
            // not bad for small data sets but could be an issue in big data sets
            var allLands = await context.Lands.Include(x => x.Type).ToListAsync();
            return [.. allLands.Where(x => x.Colors.ContainsAny(producesAny) && x.Colors.DoesNotContainsAny(doesNotProduceAny)).ToList()];
        }
    }
